"""Create detailed views for INDIRECT FREE-KICK setpieces
...
def upgrade() -> None:
    """Create v_indirect_free_kicks_detailed view"""
    
    op.execute("""
    DROP VIEW IF EXISTS v_indirect_free_kicks_detailed CASCADE
    """)
    
    op.execute("""
    CREATE VIEW v_indirect_free_kicks_detailed AS
    SELECT 
        -- Core
        ...
        -- INDIRECT FREE-KICK SPECIFIC DATA
        (sp.indirect_free_kick->>'type')::text as indirect_free_kick_type,
        (sp.indirect_free_kick->>'outcome')::text as indirect_free_kick_outcome,
        (sp.indirect_free_kick->>'link')::text as indirect_free_kick_link,
        (sp.indirect_free_kick->>'short')::boolean as indirect_free_kick_is_short,
        sp.indirect_free_kick->'attackers' as indirect_free_kick_attackers_json,
        sp.indirect_free_kick->'defenders' as indirect_free_kick_defenders_json,
        sp.indirect_free_kick->'meta' as indirect_free_kick_meta_json,
        ...
        -- Raw JSONB
        sp.indirect_free_kick as full_indirect_free_kick_jsonb
...
def downgrade() -> None:
    """Drop v_indirect_free_kicks_detailed view"""
    op.execute("DROP VIEW IF EXISTS v_indirect_free_kicks_detailed CASCADE")
