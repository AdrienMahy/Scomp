"""Fix v_events_full view - extract from entity JSON instead of null columns.

Revision ID: 024_fix_events_full_view
Revises: 023_fix_positions_extraction
Create Date: 2026-09-07 11:40:00.000000
...
revision = '024_fix_events_full_view'
down_revision = '023_fix_positions_extraction'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    # Create corrected view with entity JSON extraction
    op.execute("""
    ...
    SELECT 
        ...
        (e.entity->>'distance_gained')::FLOAT as distance_gained_meters,
        (e.entity->>'distance_gained_pct')::FLOAT as distance_gained_percent,
        (e.entity->>'available_time')::FLOAT as available_time_seconds,
        (e.entity->>'receiver_available_time')::FLOAT as receiver_available_time_seconds,
        
        -- Pressure context
        e.entity->>'receiver_pressure_level' as receiver_pressure_level,
        (e.entity->>'receiver_under_pressure')::BOOLEAN as receiver_under_pressure,
        ...
        -- Receiver context
        (e.entity->>'n_players_to_bypass_at_start')::INTEGER as n_players_to_bypass_start,
        (e.entity->>'n_players_to_bypass_at_end')::INTEGER as n_players_to_bypass_end,
        (e.entity->>'n_players_bypassed')::INTEGER as n_players_bypassed,
        ...
        (e.entity->>'period_id')::INTEGER as entity_period_id,
...
def downgrade() -> None:
    ...
    # Restore original (broken) view structure for downgrade
    op.execute("""
    CREATE OR REPLACE VIEW v_events_full AS
    SELECT 
        e.id,
        ...
        t.name as team_name,
        ot.name as opponent_team_name,
        e.entity->>'gata_display_name' as gata_display_name,
        e.time->>'seconds' as time_seconds,
        e.phase->>'name' as phase_name,
        e.spatial->>'x' as spatial_x,
        e.spatial->>'y' as spatial_y,
        e.actors->>'primary' as primary_actor,
        e.receiver->>'name' as receiver_name,
        e.channel->>'name' as channel_name,
        e.adds->>'value' as adds_value,
        e.pass_data->>'passType' as pass_type,
        e.custom->>'type' as custom_type,
        e."cross"->>'type' as cross_type,
        e.shot->>'result' as shot_result,
        e.created_at,
        e.updated_at
    FROM events e
    LEFT JOIN games g ON e.game_id = g.id
